...
AS $function$
	declare
			aw_period bpd_award_period.bpd_award_period%rowtype;
			v_last_run_t bpd_citizen.bpd_citizen_ranking.ranking_date_t%type;
		begin

		select
			case
				when min(bcr.ranking_date_t) is null then CURRENT_DATE-1000
				else min(bcr.ranking_date_t)
			end as v_last_run_t
		into
			v_last_run_t
		from
		...
		select wt.id_trx_acquirer_s,
			...
			from bpd_citizen.bpd_citizen bc
				inner join public.dblink('bpd_winning_transaction_remote',
					format('select id_trx_acquirer_s,trx_timestamp_t,acquirer_c,acquirer_id_s,operation_type_c,award_period_id_n, hpan_s, score_n, amount_i, insert_date_t, update_date_t, fiscal_code_s, correlation_id_s from bpd_winning_transaction.bpd_winning_transaction master where (master.insert_date_t between %L and CURRENT_TIMESTAMP or master.update_date_t between %L and CURRENT_TIMESTAMP) and master.enabled_b = true and master.elab_ranking_b = false
						...
						)',v_last_run_t,v_last_run_t)) AS wt(id_trx_acquirer_s varchar,trx_timestamp_t timestamptz,acquirer_c varchar, acquirer_id_s varchar, operation_type_c varchar,award_period_id_n bigint, hpan_s varchar, score_n numeric, amount_i numeric, insert_date_t timestamptz, update_date_t timestamptz, fiscal_code_s varchar, correlation_id_s varchar) on wt.fiscal_code_s=bc.fiscal_code_s
